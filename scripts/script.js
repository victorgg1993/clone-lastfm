...
function load_estil_musical(lista_canciones, estil) {
  let retorno = [];
  lista_canciones.map((elemento) =>
    elemento.genre == estil ? retorno.push(elemento) : 0
  );
...
function loadTenListened(lista_canciones) {
  let arr_cancons = lista_canciones.sort((a, b) => b.listeners - a.listeners);
  arr_cancons = arr_cancons.slice(0, 10);
...
function loadBiggest(lista_canciones) {
  ...
  bandes.map((
    banda,
    i // si la música pertany a la banda, sumem les reproduccions al nº total de la banda
  ) =>
    lista_canciones.map((musica) =>
      musica.artist.name == banda
        ? (temporal[i].reproduccions += parseInt(musica.listeners))
        : 0
    )
  );

  let biggest_band = temporal.sort(
    (a, b) => b.reproduccions - a.reproduccions
  )[0];
  temporal = [];

  lista_canciones.map((musica) =>
    musica.artist.name == biggest_band.banda ? temporal.push(musica) : 0
  );
...
function pintar_titulo_musica(evento) {
  let obj_titol_h2 = document.querySelector("h2");
  let nou_titol = evento.srcElement.innerHTML;

  if (obj_titol_h2.innerHTML != undefined && nou_titol != undefined) {
    obj_titol_h2.innerHTML = nou_titol;
  }
}
